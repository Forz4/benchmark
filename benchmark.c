...
void print_help()
{
    ...
    printf("          [-s tps]      transaction per second , set to 10 by default\n");
    printf("          [-t time]     to specify total time last , set to 10 by default , valid range [0,999999] , 0 means infinite time \n");
    printf("          [-u level]    to specify log level , valid range [0,6] , set to 2-LOGERR by default\n");
...
int start_listen()
{
    ...
    for ( i = 0 ; i < g_numOfRecv ; i ++){
        ...
        logp( LOGINF , "local_port[%d] accept OK",g_localPorts[i]);
    }
    return 0;
}
void start_send_proc()
{
    ...
    for ( i = 0 ; i < g_numOfSend ; i ++) {
        ...
        logp( LOGINF , "connect to remote port[%d] OK" ,  g_remotePorts[i]);
    }
        ...
        if ( rc < 0 ) {
            ...
            for ( i = 0 ; i < g_numOfRecv ; i ++) {
                if ( FD_ISSET(g_sock4recv[i] , &fds) ) {
		            ...
		            if (recvlen <= 0){
                        clean_recv_proc();
                        logp( LOGINF , "receiving socket fail");
                        clean_recv_proc();
                        exit(0);
		            } else {
		            	textlen = atoi(buffer);
...
void signal_handler(int no)
{
    switch(no) {
        case SIGALRM :
            logp( LOGINF , "sending process time out");
            clean_send_proc();
            exit(0);
            break;
        case SIGCHLD :
            logp( LOGERR , "detect receiving process time out");
            clean_send_proc();
            exit(0);
            break;
    }
...
int main(int argc , char *argv[])
{
    ...
    while ( ( op = getopt( argc , argv , "hf:i:r:l:s:t:u:") ) > 0 ) {
        switch(op){
            case 'h' :
                print_help();
                exit(0);
